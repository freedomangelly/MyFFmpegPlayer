...
Java_com_liuy_myffmpegplayer_ffmpegplayer_MediaPlayerJNI_nPrepareAsync(JNIEnv *env, jobject instance, jstring url_) {
...
extern "C"
JNIEXPORT void JNICALL
Java_com_liuy_myffmpegplayer_ffmpegplayer_MediaPlayerJNI_nPrepare(JNIEnv *env, jobject instance, jstring url_) {
    const char *url = env->GetStringUTFChars(url_, 0);
    if (pFFmpeg == NULL) {
        pJniCall = new DZJNICall(pJavaVM, env, instance);
        pFFmpeg = new DZFFmpeg(pJniCall, url);
